%{
_DE VERSION OF CODE:
This version of the code changes the normalization term.
    - As stated in original comments, the original normalization was chosen
    s.t. the integral over the unit circle is unity. This meant that all
    the terms where divided by sqrt(pi).
    - For our purposes, we would like to easily decompose the maps into RMS
    values. As such, the terms must be normalized by their individual RMS
    contributions. This can be done as follows:
        - Calculate the RMS value over the unit circle for each mode
        - Divide each mode but its RMS value
      It was found that the result of the above process is the same value
        as the original normalization, except without the extra division by
        sqrt(pi). 
      AS SUCH:
        This code removes that sqrt(pi).

*--** THE NET EFFECT IS: --**--**--**--**--**--**--**--**--**--**--**--**--
        Coefficients returned by this (_DE) function are the RMS
        contribution of each zernike mode. Decomposition can still be done
        as before. To calculate the RMS value of a surface, you can do a
        regular RMS over the full plane. OR, you can add the coefficients
        of the decomposition in quadrature.
%}
