...
% 
filetag = '73BWS_Var780Dat6_fullCube';
% Sections to Run
Sec2Run = [ 
    false;          %Section 1  ** etaS linear plot
    false;          %Section 2  ** etaS ylog plot
    false;          %Section 3  ** etaS xylog plot
    false;          %Section 4  ** etaP linear plot
    false;          %Section 5  ** etaP ylog plot
    false;          %Section 6  ** etaP xylog plot
    false;          %Section 7  ** donut plots
...
%% Section 1: Eta S linear plot
if Sec2Run(1)
	disp("______Section 1:______")
    
    %- Get the normalized data (full eta map)
    nmFine  = ['071019_VAR1test' filesep filetag];
    
    nrmFine = nan(length(BWs));
    eta_sFine = nan(length(BWs));
    indFine = nan(length(BWs));
    ...
    fighAsym = VFN_An_fitsDisp(nrmFine, BWs, eta_sFine, figh);
