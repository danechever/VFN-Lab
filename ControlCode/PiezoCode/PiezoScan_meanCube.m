...
*** This is the version which saves the mean of the Nread datapoints.
    - There is a newer version which saves a [X, Y, Nread, Focus]
      dimensional cube which has each of the individual read points.
%}
...
svFld = 'C:\Users\AOlab1\Desktop\DE2\VFN\PupilVFNCoupling\091918_PZT5';

% Experiment name for figures
expNm = '5PSF_CubeSaveTest';
...
%Step sizes in Volts
refStep   = 7; % refStep is the step size for a full PSF with a 10*10 grid
...
meas=zeros(Xpoints+1,Ypoints+1,length(distZ)); % list where results will be stocked
...
for k = 1:length(distZ)
    ...
    imagesc((distX-Xcenter),(distY-Ycenter),transpose((meas(:,:,k))))
...
fits.createImg(fitmap,'double',[Xpoints+1 Ypoints+1 length(distZ)]);
%header data
...
fits.writeImg(fitmap,permute(meas,[2,1,3]));
fits.closeFile(fitmap);

%% Mins and Maxs
mnVal= min(min(min(meas(Xpoints/5:4*Xpoints/5+1,Ypoints/5:4*Ypoints/5+1,:))))
mxVal = max(max(max(meas)))
floor(find(meas>= mxVal)/(Xpoints*Ypoints))
mxVal/mnVal