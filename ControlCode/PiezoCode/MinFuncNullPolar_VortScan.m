function pwr = MinFuncNullPolar_VortScan(X, MDT, zabs, cent, nrmFac)
...
X = X./[1, 1, nrmFc, nrmFac, nrmFac];    % Rescale with nrmFac
...
if ~(11.5<=vorX && vorX<=13.5)      % range semi-arbitrarily chosen
    % Z-Zaber position is beyond range
    pwr = 1000; 
    flg = true;
    %error('Desired vorX position (%f) is beyond allowable range', vorX)
end
if ~(11.5<=vorY && vorY<=14.5)      % range semi-arbitrarily chosen
    % Z-Zaber position is beyond range
    pwr = 1000; 
    flg = true;
