...
*** There is newer code which enables raster stuff with the vortex
%}
...
svFld = 'C:\Users\AOlab1\Desktop\DE2\VFN\PupilVFNCoupling\100418_VRT2';

% Experiment name for figures
expNm = '3Don_StartOfDay3';

% Flag to enable zaber motion
isZab = false;
...
%Zaber status
if isZab
    %Zabers were used so save all their params
    fits.writeKey(fitmap, 'isZab ',  'True');
    fits.writeKey(fitmap, 'VXcent',  VXcenter);
    fits.writeKey(fitmap, 'VYcent',  VYcenter);
else
    fits.writeKey(fitmap, 'isZab ',  'False');
end
...
fprintf('\nFrame for max:        %d',ind)
fprintf('\nMax to Min ratio:     %f', mxVal/mnVal)
