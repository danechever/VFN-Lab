...
svFld = 'C:\Users\AOlab1\Desktop\DE2\VFN\PupilVFNCoupling\101718_FTO3';

% Experiment name for figures
expNm = '26Don_RefForVort3';
...
VXcenter = 12.471445-0.075;      % Value when PM stage was added: 12.271445mm
VYcenter = 13.034083+0.075;  %!! must be >13.134083-0.8
Vbacklash= 0.005;
...
vStepRange = min(0.2,0.8);   %Vortex will be scanned +/- this value
                    % ie. scan will be: 
...
refStep   = 0.5; % refStep is the step size for a full PSF with a 10*10 grid
...
fits.setCompressionType(fitmap,'NOCOMPRESS');
fits.writeImg(fitmap,permute(scales,[2,1,3,4,5,6]));
fits.closeFile(fitmap);
...
fits.setCompressionType(fitmap,'NOCOMPRESS');
fits.writeImg(fitmap,permute(measScl,[2,1,3,4,5,6]));
fits.closeFile(fitmap);
