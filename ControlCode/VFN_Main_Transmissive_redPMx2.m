%{ 
 Main_Transmissive
 Code for doing a scan with the VFN Transmissive bench using only redPMs
 - Uses the PIstages for fiber X, Y, and Z
 - Uses Zabers for vortex X and Y
 - Uses the redPM and a Zaber for in-beam normalization
 - USES THE redPM for through-fiber power measurements

** Based off VFN_Main_Transmissive_PIStages
  * This script replaces the femto read with a second redPM. This is
  because we're seeing some weird stuff when measuring the superK power
  with the femto. Hopefully using the redPM for the nulling-fiber power
  measurements will fix this so that we can get the broadband VFN
  measurements.
...
expNm = '234Don_650sKEXT1';

%-- Custom auto save text
% Change at every run
run_msg = 'Repeat #233 to check system with superK EXT-4 as the source.';
% Change when doing new set of experiments
set_msg = 'Polychromatic tests with superK EXT4 - ON REDPM STILL.';
las_msg = 'superK EXT-4 100%, 650x50nm BW';   % Laser/Varia settings
flt_msg = 'None';                 % Optical Filter used
...
Zcenter   = 9.485606;% MMF: 9.193358;  % [mm] Focus (Z) center point
Zpoints   = 1;           % Number of focci taken (exact number; no longer +1)
ZStepSize = 0.005;      % Step size in mm

% Fiber X/Y center of scan in mm
Xcenter = 7.384317;% MMF: 7.405309;  % [mm]
Ycenter = 7.090550;% MMF: -9.856926; % [mm]
...
%~~ MMF NORMALIZATION STUFF
isMMFNorm = false;
%~~ END MMF NORMALIZATION

%~~ RED (NORM) POWER METER STUFF 
pmNread = 100;      % Number of samples to take
isPMNorm = true;   % SHOULD ALWAYS BE TRUE FOR THIS PARTICULAR SCRIPT VERSION
...
% Add a sizeable delay so the the redPM has time to settle before every
% reading
Delay = 0.3;
...
fprintf('---Performing redPM Setup\n')
%-- Connect to red PM only if it will be used
if isPMNorm
    VFN_setUpTWOPM100Ds
    
    % Set operating wavelength for both power meters
    VFN_PM100D_setWavelength(normPM, pmCalWvl);
    VFN_PM100D_setWavelength(nullPM, pmCalWvl);
end
...
fprintf(fileID, '        Femto:         FMTO_scale = %s; Nread = %s\r\n', 'N/A','N/A');
...
meas= nan(length(distX),length(distY),pmNread,length(distZ),length(distVX), length(distVY)); 
        ...
        for k=1:length(distZ)
            ...
            for j=1:Xpoints+1
                ...
                for i=1:Ypoints+1
                    ...
                    % Let redPM reading settle at new position
                    pause(Delay);
                                       
                    % Take a sample reading at current power
                    read = VFN_PM100D_read(nullPM, pmNread);

                    % Store data
                    meas(j,i,:,k,a,b) = read;
                    scales(j,i,1,k,a,b) = nan; % gain not needed
                    scales(j,i,2,k,a,b) = nan; % scaling factor not needed
                    
                    % Bias not needed but set to 0 for simplicity
                    locBias = 0;
...
if isMMFNorm
    ...
    %Use the nulling redPM to collect the total power, same as collecting data
    mmf_read = VFN_PM100D_read(nullPM, pmNread);
    
    % Record femto gain setting during MMF read
    MMF_FMTO_scale = nan;    
else
    %-- Set mmf_read and MMF_FMTO_scale to -9999 if MMF is not used
    mmf_read = ones(pmNread,1)*-9999;
    MMF_FMTO_scale = -9999;
end

%-- Use the norm red power meter to measure the power
if isPMNorm
    ...
    for i = 1:NTries
        try 
            ...
            pmRead = VFN_PM100D_read(normPM, pmNread);
        catch ME
...
if isPMNorm
    ...
    VFN_cleanUpTWOPM100Ds;
end

%% Compensate for fiber effects
...
%-- Take average at each sample point and rescale to uW for simplicity
measScl = mean(measScl,3)*10^6;
...
fits.writeKey(fitmap, 'Nread ', pmNread);
...
% Power meter level
fits.writeKey(fitmap, 'isAutScl', 'N/A');
...
fits.writeKey(fitmap, 'NormScl', 'N/A');
...
fprintf('\nMin in center region (un-normed): %f [uW]',mnVal)
fprintf(['\n Min Location: ', ...
...
fprintf('\nOverall Max (un-normed):          %f [uW]',mxVal2)
...
mxVal = max(meas3(:)); 
fprintf('\nMax in min Frame (un-normed):     %f [uW]', mxVal)
...
fprintf('\nPower on Fiber tip:      %f [uW]', 1/nrmVal);
